...
// $dosql->Execute("SELECT pay_time,id from pmw_order");
// while($row=$dosql->GetArray()){
//   $ymd=date("Y-m-d",$row['pay_time']);
//   $id=$row['id'];
//   $dosql->ExecNoneQuery("UPDATE pmw_order set ymd='$ymd' where id=$id");
// }
...
class wxcode {
    ...
    public function __construct($appid = 'wx389066b94450baf1', $secret = 'df59e490a31b0e2eeb41540d09d58cb7', $id = 0, $width = '430')
    {
        $this->appid = $appid;
        $this->secret = $secret;
        $this->id = $id;
    ...
    public function get_qrcode() {
        ...
        $data = array();
        $data['scene'] = "id=".$id;
        $data['page'] = "pages/play/index";  //参数跳转到product/show，产品详情
